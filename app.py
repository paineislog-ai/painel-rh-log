# -*- coding: utf-8 -*-
# ============================================================
# Painel de RH — LOG (multi-meses via Índice no Google Sheets)
# ============================================================
...
# ------------------ CONFIG ------------------
st.set_page_config(page_title="Painel de RH — LOG", layout="wide")
st.title("Painel de RH — LOG")
...
def read_training_month(file_id: str, ym_from_index: str) -> pd.DataFrame:
    ...
    out["IS_CONVOCADO"] = out["CONVOCADO"].map(
        lambda v: str(v).strip().upper() in {"1", "S", "SIM", "TRUE", "Y", "YES"}
    )
    out["IS_PRESENTE"] = out["PRESENCA"].map(
        lambda v: str(v).strip().upper() in {"1", "S", "SIM", "TRUE", "Y", "YES"}
    )
...
# ============================================================
# REGRAS LIDERANÇA (LOG)
# - Supervisor: Eleilson → Timon, Presidente Dutra, Açailândia
# - Gerente: Marta → (Eleilson + Carolina + São Luis)
# - Gerente geral: Helano → todas as cidades (fallback)
# ============================================================

SUP_ELEILSON = "ELEILSON DE SOUSA ADELINO"
GER_MARTA = "MARTA OLIVEIRA COSTA RAMOS"
GER_GERAL = "CARLOS HELANO FERREIRA DE CARVALHO"

ELEILSON_CIDADES = {"TIMON", "PRESIDENTE DUTRA", "AÇAILÂNDIA", "ACAILANDIA"}
MARTA_CIDADES = ELEILSON_CIDADES.union({"CAROLINA", "SÃO LUIS", "SAO LUIS"})

def infer_lider(cidade: str, supervisor: str) -> str:
    sup = _upper(supervisor)
    cid = _upper(cidade)

    # se a base já traz alguém, respeita (exceto vazio/None)
    if sup and sup not in {"NAN", "NONE"}:
        return sup

    # fallback por cidade
    if cid in ELEILSON_CIDADES:
        return SUP_ELEILSON
    if cid in MARTA_CIDADES:
        return GER_MARTA

    # padrão: gerente geral
    return GER_GERAL
...
df["GERENTE"] = df.apply(lambda r: infer_lider(r.get("CIDADE", ""), r.get("SUPERVISOR", "")), axis=1)

df_train = pd.concat(all_train, ignore_index=True) if all_train else pd.DataFrame()
if not df_train.empty:
    df_train["GERENTE"] = df_train["CIDADE"].apply(lambda c: infer_lider(c, ""))
...
with tab_people:
    ...
    ppl["IDADE"] = ppl["NASCIMENTO"].apply(lambda x: (pd.Timestamp(end_d) - to_ts(x)).days / 365.25 if not pd.isna(to_ts(x)) else np.nan)
    ppl["TEMPO_CASA_MESES"] = ppl["ADMISSAO"].apply(lambda x: (pd.Timestamp(end_d) - to_ts(x)).days / 30.4375 if not pd.isna(to_ts(x)) else np.nan)
...
# ============================================================
with tab_gest:
    st.markdown("<div class='section'>Estrutura por liderança (fim do período)</div>", unsafe_allow_html=True)
    ...
    adm_g["GERENTE"] = adm_g.apply(lambda r: infer_lider(r.get("CIDADE", ""), r.get("SUPERVISOR", "")), axis=1)
    dem_g["GERENTE"] = dem_g.apply(lambda r: infer_lider(r.get("CIDADE", ""), r.get("SUPERVISOR", "")), axis=1)
    mov_g = (
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.altair_chart(bar_with_labels(hc_g, "GERENTE", "HC_FIM", height=320, title="Headcount por liderança (fim)"), use_container_width=True)
    with c2:
        tmp = mov_g.copy()
        if len(tmp):
            ...
            ).properties(height=320, title="Movimentações por liderança (período)")
            st.altair_chart(chart, use_container_width=True)
        else:
    ...
    with c3:
        ...
        st.altair_chart(bar_with_labels(abs_plot, "GERENTE", "ABS_%", height=320, title="Absenteísmo por liderança (%)", y_format=".2f"), use_container_width=True)
    with c4:
        turn_plot = turn_g.sort_values("TURN_%", ascending=False).copy()
        turn_plot["TURN_%"] = turn_plot["TURN_%"].fillna(0)
        st.altair_chart(bar_with_labels(turn_plot, "GERENTE", "TURN_%", height=320, title="Turnover por liderança (%)", y_format=".2f"), use_container_width=True)

    st.markdown("<div class='section'>Regras (fixas)</div>", unsafe_allow_html=True)
    st.write("Eleilson de Sousa Adelino (supervisor) → Timon, Presidente Dutra, Açailândia")
    st.write("Marta Oliveira Costa Ramos (gerente) → Timon, Presidente Dutra, Açailândia, Carolina, São Luis")
    st.write("Carlos Helano Ferreira de Carvalho (gerente geral) → todas as cidades (fallback)")
...
with tab_train:
    ...
    if df_train is None or df_train.empty:
        ...
        if tr.empty:
            ...
            by_city = tr.groupby("CIDADE").agg(CONVOCADOS=("IS_CONVOCADO", "sum"), PRESENTES=("IS_PRESENTE", "sum")).reset_index()
            ...
            by_g = tr.groupby("GERENTE").agg(CONVOCADOS=("IS_CONVOCADO", "sum"), PRESENTES=("IS_PRESENTE", "sum")).reset_index()
            ...
            colA, colB = st.columns(2)
            with colA:
                st.altair_chart(bar_with_labels(by_city, "CIDADE", "COB_%", height=320, title="Cobertura de treinamento por cidade (%)", y_format=".1f"), use_container_width=True)
            with colB:
                st.altair_chart(bar_with_labels(by_g, "GERENTE", "COB_%", height=320, title="Cobertura de treinamento por liderança (%)", y_format=".1f"), use_container_width=True)
...
with tab_base:
    ...
    try:
        ...
        st.download_button(
            ...
            data=xbuf,
            file_name=f"rh_recorte_log_{ym_sel}.xlsx",
